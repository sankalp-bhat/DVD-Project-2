...
def run_leakage_simulation(pvt_row):
    ...
    .END
    """
    with open('../netlists/nand2_leakage.sp', 'w') as f:
        f.write(netlist)
    
    # subprocess.run(['ngspice', '-b', 'nand2_leakage.sp'])
    command = "ngspice -b ../netlists/nand2_leakage.sp > ../netlists/nand2_leakage.txt"
    subprocess.run(command, shell=True)

    # with open('nand2_leakage.txt', 'r') as f:
    #     lines = f.readlines()
    #     parsed_data = {}
    #     for line in lines:
    #         if '=' in line:
    ...
    with open('../netlists/nand2_leakage.txt', 'r') as f:
...
def run_delay_simulation(pvt_row):
    ...
    * Definition of type of analysis
    ...
    with open('../netlists/nand2_delay.sp', 'w') as f:
        f.write(netlist)

    command = "ngspice -b ../netlists/nand2_delay.sp > ../netlists/nand2_delay.txt"
    subprocess.run(command, shell=True)
  
    # Tokenization
    with open('../netlists/nand2_delay.txt', 'r') as f:
        lines = f.readlines()
...
results.to_csv('../outputs/simulation_results_nand.csv', index=False)